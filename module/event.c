...
DECL_HANDLER(query_event)
{
    ...
    if (!(event = get_event_obj( current_thread->process, req->handle, EVENT_QUERY_STATE ))) return;
...
DECL_HANDLER(create_keyed_event)
{
    ...
    if (objattr->rootdir && !(root = get_directory_obj( current_thread->process, objattr->rootdir, 0 ))) return;
...
DECL_HANDLER(open_keyed_event)
{
    ...
    get_req_unicode_str( &name );
    if (req->rootdir && !(root = get_directory_obj( current_thread->process, req->rootdir, 0 ))) return;
