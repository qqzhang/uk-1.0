					 ...
					 struct console_renderer_event* evt)
{
    ...
    /* try to collapse evt into current_thread queue's events */
    if (evts->num_used)
    {
...
void inherit_console(struct thread *parent_thread, struct process *process, obj_handle_t hconin)
{
    ...
    /* if parent is a renderer, then attach current_thread process to its console
     * a bit hacky....
     */
    if (hconin)
    {
...
static struct console_input* console_input_get( obj_handle_t handle, unsigned access )
{
    ...
    if (handle)
	console = (struct console_input *)get_handle_obj( current_thread->process, handle,
							  access, &console_input_ops );
    else if (current_thread->process->console)
    {
	console = (struct console_input *)grab_object( current_thread->process->console );
    }
...
static int get_console_mode( obj_handle_t handle )
{
    ...
    if ((obj = get_handle_obj( current_thread->process, handle, FILE_READ_PROPERTIES, NULL )))
    {
...
static int set_console_mode( obj_handle_t handle, int mode )
{
    ...
    if (!(obj = get_handle_obj( current_thread->process, handle, FILE_WRITE_PROPERTIES, NULL )))
        return 0;
    if (obj->ops == &console_input_ops)
    {
...
static int read_console_input( obj_handle_t handle, int count, int flush )
{
    ...
    if (!(console = (struct console_input *)get_handle_obj( current_thread->process, handle,
				   ...
				   const WCHAR *title, data_size_t len )
{
	...
	screen_buffer = (struct screen_buffer *)get_handle_obj( current_thread->process, req->active_sb,
                                    ...
                                    const struct set_console_output_info_request *req )
{
	...
	if (current_thread->process->console != screen_buffer->input)
	{
	    set_error( STATUS_INVALID_PARAMETER );
	    return 0;
	}
...
DECL_HANDLER(alloc_console)
{
    ...
    {
    case 0:
        /* renderer is current_thread, console to be attached to parent process */
        renderer = current_thread;
        if (!(process = current_thread->process->parent))
        {
        ...
        break;
    case 0xffffffff:
        /* no renderer, console to be attached to current_thread process */
        renderer = NULL;
        ...
        break;
    default:
        /* renderer is current_thread, console to be attached to req->pid */
        renderer = current_thread;
    ...
    if (req->input_fd != -1)
    {
        if ((fd = thread_get_inflight_fd( current_thread, req->input_fd )) == -1)
        {
            set_error( STATUS_INVALID_PARAMETER );
            goto the_end;
        }
    ...
    {
        if ((in = alloc_handle( current_thread->process, console, req->access, req->attributes )))
        {
            if (!console->evt ||
                (evt = alloc_handle( current_thread->process, console->evt, SYNCHRONIZE|GENERIC_READ|GENERIC_WRITE, 0 )))
            {
                if (attach)
                {
                    process->console = (struct console_input*)grab_object( console );
            ...
            close_handle( current_thread->process, in );
        }
        release_object( console );
    }
 the_end:
...
/* free the console of the current_thread process */
DECL_HANDLER(free_console)
{
    free_console( current_thread->process );
}
...
DECL_HANDLER(get_console_renderer_events)
{
    ...
    evt = (struct console_input_events *)get_handle_obj( current_thread->process, req->handle,
...
DECL_HANDLER(open_console)
{
    ...
    if (!req->from)
    {
        if (current_thread->process->console)
            obj = grab_object( (struct object*)current_thread->process->console );
    }
    else if (req->from == (obj_handle_t)1)
    {
        if (current_thread->process->console && current_thread->process->console->active)
            obj = grab_object( (struct object*)current_thread->process->console->active );
    }
    else if ((obj = get_handle_obj( current_thread->process, req->from,
...
DECL_HANDLER(write_console_input)
{
    ...
    reply->written = 0;
    if (!(console = (struct console_input *)get_handle_obj( current_thread->process, req->handle,
...
DECL_HANDLER(create_console_output)
{
    ...
    if (screen_buffer)
    {
        ...
        reply->handle_out = alloc_handle( current_thread->process, screen_buffer, req->access, req->attributes );
        release_object( screen_buffer );
    }
    release_object( console );
}
...
DECL_HANDLER(set_console_output_info)
{
    ...
    if ((screen_buffer = (struct screen_buffer*)get_handle_obj( current_thread->process, req->handle,
                                                                FILE_WRITE_PROPERTIES, &screen_buffer_ops)))
    {
...
DECL_HANDLER(send_console_signal)
{
    ...
    group = req->group_id ? req->group_id : current_thread->process->group_id;

    if (!group)
        set_error( STATUS_INVALID_PARAMETER );
    else
        propagate_console_signal( current_thread->process->console, req->signal, group );
}

/* get console which renderer is 'current_thread' */
static int cgwe_enum( struct process* process, void* user)
{
    if (process->console && process->console->renderer == current_thread)
    {
...
DECL_HANDLER(get_console_wait_event)
{
    ...
    if (current_thread->process->console)
        console = (struct console_input*)grab_object( (struct object*)current_thread->process->console );
    else enum_processes(cgwe_enum, &console);

    if (console)
    {
        reply->handle = alloc_handle( current_thread->process, console->event, EVENT_ALL_ACCESS, 0 );
        release_object( console );
    }
