...
#ifdef CONFIG_UNIFIED_KERNEL
    assert( spin_is_locked(&thread_lock.lock) );
    recursive_spin_unlock_bh(&thread_lock);
    wait = mem_alloc( FIELD_OFFSET(struct thread_wait, queues[count]) );
                            ...
                            int flags, timeout_t timeout )
{
    ...
    {
    case SELECT_NONE:
        if (!wait_on( select_op, 0, NULL, flags, timeout )) goto done;
        break;
    ...
    case SELECT_KEYED_EVENT_RELEASE:
        object = (struct object *)get_keyed_event_obj( current_thread->process, select_op->keyed_event.handle,
                         select_op->op == SELECT_KEYED_EVENT_WAIT ? KEYEDEVENT_WAIT : KEYEDEVENT_WAKE );
        if (!object) goto done;
        ret = wait_on( select_op, 1, &object, flags, timeout );
        release_object( object );
        if (!ret) goto done;
        current_thread->wait->key = select_op->keyed_event.key;
        break;
                            ...
                            int flags, timeout_t timeout )
{
    ...
    case SELECT_SIGNAL_AND_WAIT:
        ...
        if (select_op->signal_and_wait.signal)
        {
            ...
            /* check if we woke ourselves up */
            if (!current_thread->wait) return timeout;
        }
        break;
