...
static unsigned int get_image_params( struct mapping *mapping, int unix_fd, int protect )
{
    ...
    mapping->cpu = current_thread->process->cpu;
    switch (mapping->cpu)
    {
    case CPU_x86:
...
DECL_HANDLER(get_mapping_info)
{
    ...
    if (!(mapping = get_mapping_obj( current_thread->process, req->handle, req->access ))) return;

    if ((mapping->protect & VPROT_IMAGE) && mapping->cpu != current_thread->process->cpu)
    {
    ...
    {
        if (!is_fd_removable(fd)) reply->mapping = alloc_handle( current_thread->process, mapping, 0, 0 );
        release_object( fd );
    }
    if (mapping->shared_file)
    {
